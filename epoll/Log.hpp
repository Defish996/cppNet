...
public:
    ...
    void LogMessage(int level, const char *format, ...)
    {
        ...
        va_list args;           // const char *, va_等的是用来对付可变参数的函数
        va_start(args, format); // 初始化()
        // args指向了可变参数部分
        ...
        // snprintf 用于将格式化的数据写入一个固定大小的缓冲区，并且保证不会超过指定的最大长度（包括终止的空字符 \0）。
        // 如果格式化的结果超过了缓冲区的大小，超出的部分会被截断，但总是会在缓冲区的末尾添加一个空字符来确保它是以 null 结尾的字符串
