...
std::string HandlerHTTPRequest(std::string &request) // 接受的参数假设我们已经读到了一个完整的请求
{
    ...
    {
        // 进行注册操作
        std::string sessionid = onlineuser.AddSession(name, passwd);
    ...
    // code = 301;// 临时重定向, 会重定向到新的url, 这个新的url放在location中
    ...
    std::string passwordcookie = "Set-Cookie: password=123456\r\n";
    resp.AddHeader(passwordcookie); 
    std::string statuscookie = "Set-Cookie: status=logind\r\n";
